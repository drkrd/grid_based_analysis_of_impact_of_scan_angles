...
#opt_filter(lasc) <- "-keep_first -drop_z_below 1"
...
met_calc = function(cluster, res)
{
  ...
  las = readLAS(cluster, filter = "-keep_first -drop_z_below 1")
...
output_p70_3  <- catalog_apply(lasc, met_calc, res = rs, .options = opt)
